...
function normalizeKeys(obj: any, type: Type) {
    ...
    if (type === Type.SERVERPOOL) {
        ...
        if (normalized.min_vm !== undefined) {
            normalized.minVm = Number(normalized.min_vm);
            delete normalized.min_vm;
        }
        if (normalized.max_vm !== undefined) {
            normalized.maxVm = Number(normalized.max_vm);
            delete normalized.max_vm;
        }
        if (normalized.config_id !== undefined) {
            normalized.config = normalized.config_id;
            delete normalized.config_id;
        }
    }
