<script lang="ts">
  ...
  import { simpleMode, darkMode, reduceMotion, refreshInterval, language } from '$lib/store/uiStore';
...
<svelte:head><title>{$_('settings.pageTitle')}</title></svelte:head>

<div class="space-y-7 animate-fade-up">

  <h1 class="text-3xl font-bold text-primary-800 dark:text-primary-300">{$_('settings.title')}</h1>
  ...
  <div class="card card-interactive p-6 flex items-center gap-4">
    ...
    <div class="min-w-0 flex-1">
      <div class="flex items-center gap-2">
        ...
        {#if $authStore?.role === 'admin'}<span class="badge badge-admin">{$_('login.admin')}</span>{/if}
      </div>
      <p class="text-sm text-neutral-500 mt-0.5">{$_('settings.teacherAccount')}</p>
    </div>
    <button onclick={() => sshModal = true} class="btn btn-secondary text-sm shrink-0">
      ...
      {$_('settings.addSSHKey')}
    </button>
  </div>

  <!-- Préférences -->
  <div class="card p-5">
    <h2 class="text-sm font-bold text-neutral-800 dark:text-neutral-200 mb-1">{$_('settings.preferences')}</h2>
    <div class="divide-y divide-black/5 dark:divide-white/5">
      ...
      <div class="flex items-center justify-between py-3.5">
        <div class="flex items-center gap-3">
          ...
          <div>
            <p class="text-sm font-medium text-neutral-800 dark:text-neutral-200">{$_('settings.appearance')}</p>
            <p class="text-xs text-neutral-400">{$darkMode ? $_('settings.darkMode') : $_('settings.lightMode')}</p>
          </div>
        </div>
        <button onclick={() => darkMode.update(v => !v)} role="switch" aria-checked={$darkMode} aria-label={$_('settings.darkMode')}
      ...
      {#if $authStore?.role === 'admin'}
        ...
        <div class="flex items-center justify-between py-3.5">
          <div class="flex items-center gap-3">
            ...
            <div>
              <p class="text-sm font-medium text-neutral-800 dark:text-neutral-200">{$_('settings.displayMode')}</p>
              <p class="text-xs text-neutral-400">{$simpleMode ? $_('settings.simple') : $_('settings.expert')}</p>
            </div>
          </div>
          <button onclick={() => simpleMode.update(v => !v)} role="switch" aria-checked={$simpleMode} aria-label={$_('settings.displayMode')}
      ...
      <div class="flex items-center justify-between py-3.5">
        <div class="flex items-center gap-3">
          ...
          <div>
            <p class="text-sm font-medium text-neutral-800 dark:text-neutral-200">{$_('settings.reduceMotion')}</p>
            <p class="text-xs text-neutral-400">{$_('settings.reduceMotionHint')}</p>
          </div>
        </div>
        <button onclick={() => reduceMotion.update(v => !v)} role="switch" aria-checked={$reduceMotion} aria-label={$_('settings.reduceMotion')}
      ...
      {#if $authStore?.role === 'admin'}
        ...
        <div class="flex items-center justify-between py-3.5">
          <div class="flex items-center gap-3">
            ...
            <div>
              <p class="text-sm font-medium text-neutral-800 dark:text-neutral-200">{$_('settings.refresh')}</p>
              <p class="text-xs text-neutral-400">{$_('settings.refreshHint')}</p>
            </div>
          </div>
      ...
      <div class="flex items-center justify-between py-3.5">
        <div class="flex items-center gap-3">
          <svg class="w-5 h-5 text-neutral-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" d="M3 5h12M9 3v2m1.048 9.5A18.022 18.022 0 016.412 9m6.088 9h7M11 21l5-10 5 10M12.751 5C11.783 10.77 8.07 15.61 3 18.129"/></svg>
          <div>
            <p class="text-sm font-medium text-neutral-800 dark:text-neutral-200">{$_('settings.language')}</p>
            <p class="text-xs text-neutral-400">{$_('settings.languageHint')}</p>
          </div>
        </div>
        <select bind:value={$language} aria-label={$_('settings.language')} class="field text-sm w-auto py-1.5">
      ...
      <div class="flex items-center justify-between py-3.5">
        <div class="flex items-center gap-3">
          ...
          <div>
            <p class="text-sm font-medium text-neutral-800 dark:text-neutral-200">{$_('settings.account')}</p>
            <p class="text-xs text-neutral-400">{$authStore?.email ?? '—'}{#if $authStore?.role === 'admin'} · {$_('login.admin')}{/if}</p>
          </div>
        </div>
        <button onclick={logout} class="btn btn-secondary text-xs">{$_('common.logout')}</button>
      </div>
    </div>
  </div>
