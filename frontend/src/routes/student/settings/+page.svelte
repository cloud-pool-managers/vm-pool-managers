<script lang="ts">
  ...
  import { darkMode, reduceMotion, language } from '$lib/store/uiStore';
  ...
  async function addKey() {
    ...
    addingKey = true; keyMsg = ''; keyErr = false;
    try {
      ...
      if (!d.success) { keyErr = true; keyMsg = $_('student.sshErrorPrefix') + (d.error ?? 'échec'); return; }
      keyMsg = $_('student.sshSaved'); sshKey = '';
    } catch { keyErr = true; keyMsg = $_('student.sshSaveError'); }
...
<svelte:head><title>{$_('student.pageTitle')}</title></svelte:head>
...
<div class="max-w-xl mx-auto py-10 space-y-6 animate-fade-up">
  ...
  <div class="card card-interactive p-6 flex items-center gap-4">
    ...
    <div class="min-w-0 flex-1">
      <h1 class="text-xl font-bold text-neutral-900 dark:text-white truncate">{displayName || $_('student.fallbackName')}</h1>
      <p class="text-sm text-neutral-500 flex items-center gap-1.5 mt-0.5">
        ...
        {:else}
          ...
          {$_('student.connectedMoodle')}
        {/if}
      </p>
    </div>
    <button onclick={disconnect} class="btn btn-secondary text-xs shrink-0">{$_('common.logout')}</button>
  </div>
      ...
      <div class="flex items-center justify-between py-3.5">
        <div class="flex items-center gap-3">
          ...
          <div>
            ...
            <p class="text-xs text-neutral-400">{$_('settings.reduceMotionHintShort')}</p>
          </div>
        </div>
  ...
  {#if provider === 'moodle'}
    <div class="card p-5">
      <h2 class="text-sm font-bold text-neutral-800 dark:text-neutral-200 mb-1">{$_('student.sshOptional')}</h2>
      <p class="text-xs text-neutral-400 mb-3">{$_('student.sshHint')}</p>
      ...
      <div class="flex items-center justify-between mt-2">
        {#if keyMsg}<p class="text-xs {keyErr ? 'text-red-600' : 'text-green-600'}">{keyMsg}</p>{:else}<span></span>{/if}
  ...
  <a href="/student" class="btn btn-secondary text-sm w-full">{$_('student.backToCourses')}</a>
</div>
